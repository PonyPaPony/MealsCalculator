...
class MockController:
    def __init__(self, instance, language, with_attrs=False):
        with patch("main_controller.DataDefaults"), \
...
def test_open_calculate_window(controller, products_exist):
    ...
    if not products_exist:
        controller.open_calculate_window()
        controller.error_message.assert_called_once_with(_("Ошибка"), _("Нет продуктов для расчёта."))
    else:
...
def test_open_stats_menu_placeholder(controller):
    # Пока не реализовано, но метод существует
    controller.open_stats_menu()