...
widgets_all = [
    ...
    (tk.Button, {'text': 'test', 'command': MagicMock()}, {'row': 4, 'column': 0}, 'grid', {}),
    ...
    (tk.Button, {'text': 'test', "command": MagicMock()}, {'side': 'top', 'fill': 'x'}, 'pack', {}),
    ...
    (tk.Button, {'text': 'test', "command": MagicMock()}, {'width': 10, 'height': 10}, 'place', {}),
]
widgets_wid = [
    # cls, style, placement_dict, placement_type, extra_options
    ...
    (tk.Button, {'text': 'test', 'command': MagicMock()}, {'row': 4, 'column': None}, 'grid', {}),
    ...
    (tk.Button, {'text': 'test', "command": MagicMock()}, {'side': 'top', 'fill': None}, 'pack', {}),
    ...
    (tk.Button, {'text': 'test', "command": MagicMock()}, {'width': 10, 'height': None}, 'place', {}),
]
...
@pytest.mark.parametrize("cls, style, placement, placement_type, extra_options", widgets_all)
def test_factory_combined(instance, fake_root, cls, style, placement, placement_type, extra_options):
    ...
    if cls in [tk.Tk, tk.Toplevel]:
        with patch('gui_factory.set_window_icon', return_value=None):
            ...
            widget = app.create_widgets(cls=cls, frame=fake_root, style=style, **kwargs, **extra_options)
    else:
        app = instance(WidgetFactory, bg_color='#f0f8ff')
        widget = app.create_widgets(cls=cls, frame=fake_root, style=style, **kwargs, **extra_options)

    assert isinstance(widget, cls), f"Ожидался объект класса {cls}, получили {type(widget)}"
    ...
    if cls not in [tk.Tk, tk.Toplevel]:
        if extra_options.get("is_scrollable", False):
            ...
            assert isinstance(widget.master, Canvas), "Scrollable frame должен иметь Canvas в качестве master"
        else:
            assert widget.master == fake_root, "Обычные виджеты должны иметь root в качестве master"
...
@pytest.mark.parametrize("cls, style, placement, placement_type, extra_options", widgets_wid)
def test_factory_invalid_placement(instance, fake_root, cls, style, placement, placement_type, extra_options):
    """
    ...
    with pytest.raises(ValueError, match='Ошибка размещения виджета:'):
        app.create_widgets(cls=cls, frame=fake_root, style=style, **kwargs, **extra_options)
