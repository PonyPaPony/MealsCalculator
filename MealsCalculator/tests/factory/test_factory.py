...
test_container_2 = [
    ...
    (fake_root, 'invalid', "Ошибка, Некорректное значение. Используйте 'hide' или 'show'."),
]
test_container_3 = [
...
def test_window_status(instance, fake_root):
    ...
    fake_root.update()
    assert fake_root.winfo_viewable() == False

    app.window_status(fake_root, 'show')
    fake_root.update()
    assert fake_root.winfo_viewable() == True
...
def test_save_results(_, instance, tmp_path):
    ...
    app.read_and_write_file = MagicMock(side_effect = lambda path, mode, data=None: (
        [] if mode == 'r' else None
    ))
    ...
    app.error_message.assert_called_with("Ошибка", "Некорректный формат данных для сохранения.")
