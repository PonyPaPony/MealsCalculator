...
from localization import SetupLanguage, Localization
...
def test_setup_language_invalid(monkeypatch, instance):
    ...
    monkeypatch.setattr("gettext.translation", lambda *a, **k: (_ for _ in ()).throw(FileNotFoundError("not found")))

    setup = instance(SetupLanguage)
    setup.setup_language("nonexistent")

    setup._mocks['error_handler'].assert_called_once_with(_("Ошибка"), _("Файл перевода не найден."))
...
@pytest.mark.parametrize("language", ["en", "ru"])
def test_start_with_root_calls_destroy(mock_factory, mock_context, mock_manager, mock_data, mock_main, language, instance):
    cmd = instance(Localization)
    cmd.root = MagicMock()
...
@patch("localization.read_config", return_value=('ru', ['path_ru.json', 'path_en.json']))
...
@patch("localization.read_config", return_value=(None, ['path_ru.json', 'path_en.json']))
@patch.object(Localization, 'init_window')
@patch.object(Localization, 'setup_language')
@patch.object(Localization, 'start_application_with_language')
def test_run_without_saved_language(mock_start, mock_setup, mock_init, mock_read, instance):
    app = instance(Localization)
    app.run()
...
def test_init_window(instance, mock_tkinter):
    ...
    mock_tkinter['tk_instance'].title.assert_called_once_with("Выбор Языка / Set Language")
    mock_tkinter['tk_instance'].geometry.assert_called_once_with('500x350')
    mock_tkinter['tk_instance'].protocol.assert_called_once_with('WM_DELETE_WINDOW', ANY)

    # Проверяем создание Frame
    mock_tkinter['Frame'].assert_called_once_with(mock_tkinter['tk_instance'])
    mock_tkinter['frame_instance'].grid.assert_called_once_with(row=0, column=0, columnspan=1, padx=10, pady=10, sticky='nsew')
    ...
    mock_tkinter['frame_instance'].grid_columnconfigure.assert_called_once_with(0, weight=1)

    mock_tkinter['tk_instance'].grid_rowconfigure.assert_called_once_with(0, weight=1)
    mock_tkinter['tk_instance'].grid_columnconfigure.assert_called_once_with(0, weight=1)
