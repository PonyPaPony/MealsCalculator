...
from config_manager import write_config, read_config, reset_config, CONFIG_PATH
...
def test_without_config_file(fake_config):
    ...
    with patch('config_manager.CONFIG_PATH', str(fake_config)):
        ...
        assert file_paths == [], "Пути должны быть пустым списком, если конфиг отсутствует"
        logger.info("Тест: Завершен успешно.")
...
def test_read_with_missing_key(fake_config):
    ...
    with patch('config_manager.CONFIG_PATH', str(fake_config)):
        ...
        saved_lang, file_paths = read_config()
        assert saved_lang is None, "Язык должен быть None при отсутствии ключа 'language'"
        assert file_paths == [None, None], "Пути должны быть [None, None] при отсутствии ключей"
...
def test_read_config_missing_settings_section(fake_config):
    ...
    with patch('config_manager.CONFIG_PATH', str(fake_config)):
        ...
        saved_lang, file_paths = read_config()
        assert saved_lang is None, "Язык должен быть None при отсутствии секции 'settings'"
        assert file_paths == [None, None], "Пути должны быть [None, None] при отсутствии секции 'file_path'"
...
def test_read_corrupted_config(fake_config):
    ...
    with patch('config_manager.CONFIG_PATH', str(fake_config)):
        ...
        assert file_paths == [None, None], "Пути должны быть [None, None] при повреждённом файле"
...
def test_read_empty_file(fake_config):
    ...
    with patch('config_manager.CONFIG_PATH', str(fake_config)):
        ...
        assert file_paths == [None, None], "Пути должны быть [None, None] при пустом файле"
...
def test_read_config_returns_nothing(fake_config):
    ...
    with patch('config_manager.CONFIG_PATH', str(fake_config)):
        ...
        with patch('configparser.ConfigParser.read', return_value=[]):
            ...
            assert file_paths == [None, None], "Пути должны быть [None, None], если файл не прочитан"
...
def test_write_and_reset_config_contents(fake_config):
    ...
    with patch('config_manager.CONFIG_PATH', str(fake_config)):
        ...
        assert config.get('settings', 'language') == 'ru', "Язык в конфиге должен быть 'ru'"
        assert config.get('file_path',
                          'path_ru') == path_ru, "Путь к русскому файлу некорректен"

        reset_config()
        # Повторно читаем конфиг после сброса
        config = configparser.ConfigParser()
        config.read(fake_config)
        assert config.get('settings', 'language') == '', "После сброса язык должен быть пустой строкой"
