...
def test_set_window_icon(monkeypatch, fake_script_path, expected_icon_path):
    ...
    assert isinstance(actual_path, Path), f"Аргумент должен быть Path, но был {type(actual_path)}"
    assert actual_path == expected_icon_path, f"Ожидался путь {expected_icon_path}, но был {actual_path}"
