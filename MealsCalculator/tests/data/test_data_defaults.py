...
@patch('data_defaults.read_config', return_value=(['ru', 'en'], ['data/products/products_ru.json', 'data/products/products_en.json']))
...
def test_get_default_products_valid_languages(language, expected_key, temp_file_path, instance):
...
@pytest.mark.parametrize("language, expected_key", test_case)
def test_ensure_file_with_defaults_creates_new_file(language, expected_key, temp_file_path, instance):
...
def test_write_default_ioerror(mock_open, instance, temp_file_path):
    ...
    assert any("Не удалось записать" in msg for msg in error_msgs), \
        "Ожидалось сообщение о невозможности записи файла"
    assert any("Ошибка чтения" in msg for msg in error_msgs), \
        "Ожидалось сообщение об ошибке чтения файла"

    # Проверка логов
    log_msgs = [call.args[0] for call in data.log.call_args_list]
    assert any("Ошибка записи файла" in msg for msg in log_msgs), \
        "Ожидалось лог-сообщение о создании файла"
    assert any("Ошибка чтения файла" in msg for msg in log_msgs), \
