...
from product_manager import ProductManagerGUI, ProductContext
...
class ButtonCall:
    ...
    def click(self, command=0):
        try:
            ...
            raise IndexError(f"Нет кнопки с индексом {command} в create_button.call_args_list")
...
test_case_2 = [
    ...
    (fake_root, 'abc', "Неизвестный таг действия: abc", "Ошибка: Неизвестный таг действия — abc")
]
test_case_3 = [
...
def test_window_root(instance, context, fake_root, tag, method):
    ...
    other_methods = {"open_add_products_window", "open_del_products_window", "open_change_products_window"} - {method}
...
def test_open_add_products_window(instance, context, fake_root, name, kcal, key, value, title, msg):
    app = instance(ProductManagerGUI, context)
...
def test_open_change_products_window(instance, context, fake_root, name, kcal, key, value, title, msg):
    app = instance(ProductManagerGUI, context)
    btn = ButtonCall(app, True, key, value, name, kcal)
    ...
    if key and value:
        ...
        assert any(call_ == call(title, msg) for call_ in error_calls), (
