...
from product_manager import ProductManager, ProductContext
...
test_case = [
    ...
    ("en", "Успех", "Продукты сохранены!"),  # TODO: Требуется повторный тест после подключения перевода.
]
test_case_2 = [
...
test_case_6 = [
    ...
    (3.14, 14.88, {"Яблоки": 52.0}, "Ошибка: Наименование продукта не может быть числом."),
    ("Яблоко", 'abc', {"Яблоки": 52.0}, "Ошибка: Калорийность должна быть числом."),
    ("Яблоко", 0, {"Яблоки": 52.0}, "Ошибка: Калорийность должна быть положительным числом."),
    ("Яблоко", -9999, {"Яблоки": 52.0}, "Ошибка: Калорийность должна быть положительным числом."),
]
test_case_7 = [
    ("ru", {"Бананы": 100}),
    ("en", {"Bananas": 100}),
...
def test_save_products(instance, context, language, title, msg):
    ...
    manager.factory.read_and_write_file.assert_called_with(mock_env.file_path, 'w', manager.products)
...
def test_load_products_internal_file_missing(mock_exists, instance, context, language, items):
    context.language = language
    manager = instance(ProductManager, context)
    mock_env = Assistant(manager, language, mock_path=True)
...
def test_load_products_internal(instance, context, language, msg, init_language):
    context.language = init_language
    manager = instance(ProductManager, context)
    mock_env = Assistant(manager, init_language)
    with pytest.raises(ValueError, match=msg):
