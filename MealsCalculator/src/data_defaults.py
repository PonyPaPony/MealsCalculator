...
class DataDefaults:
    def __init__(self,
        ...
        # -- Listbox Settings -- #
        self.listbox_style = {
            "height": 12, 'width': 50, "font": ("Arial", 10, 'bold'), "selectmode": "tk.SINGLE"
        }
        self.listbox_grid = {
    ...
    def ensure_file_with_defaults(self, language: str,
                                  default_data_func: Optional[Callable[[], dict[str, Any]]] = None):
        supported_languages = {
        ...
        default_data_func = default_data_func or (lambda: self.get_default_products(language))
        ...
        def write_default(data: dict[str, Any]):
            try:
                ...
                self.error_message(_("Ошибка"), _("Не удалось записать {f}.").format(f=file_path))
            return data
        ...
        try:
            ...
            self.error_message(_("Ошибка"), _("Ошибка чтения {f}. Загружаем дефолтные данные").format(f=file_path))
            return write_default(default_data_func())
