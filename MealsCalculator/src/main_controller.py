...
from product_manager import ProductManager, ProductCalculator, ProductContext
from gui_factory import WidgetBuilder, Factory
...
class MainController:
    ...
    def create_buttons(self, frame):
        ...
        for idx, (text, command) in enumerate(buttons):
            ...
            self.builder.create_button(frame, text, command, style=style,
    ...
    def run(self):
        self.root = self.builder.create_widgets(cls=tk.Tk, title=_("Главное Меню"), size='500x350')
        self.root.protocol("WM_DELETE_WINDOW", lambda: self.factory.on_close(self.root))
        main_frame = self.builder.create_frame(self.root, grid={**self.settings.frame_grid})
    ...
    def open_calculate_window(self):
        ...
        def add_row():
            row = self.calculator.create_input_product_row(product_rows, scrollable_area)
        ...
        btn_frame = self.builder.create_frame(win, grid={**self.settings.frame_grid, 'row': 1})
    ...
    def open_manager_products_menu(self):
        ...
        for idx, (label, tag) in enumerate(actions):
            self.builder.create_button(frame, text=label,
                                       command=lambda t=tag: self.manager.root_for_window(win, tag=t),
                                       style={**self.settings.font_12, **self.settings.green},
                                       grid={**self.settings.button_grid, 'row': idx})

        self.builder.create_button(frame, text=_("Назад в меню"),
