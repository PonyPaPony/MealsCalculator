...
# —— Project Imports —— #
from config_manager import write_config, read_config
...
class SetupLanguage:
    ...
    """
    def __init__(self, error_handler: Optional[Callable[[str, str], None]] = None):
        """
        Args:
    ...
    def setup_language(self, language_code: str):
        ...
        try:
            lang = gettext.translation(domain='messages', localedir=locale_dir, languages=[language_code],
                                       fallback=True)
            lang.install()
            _ = lang.gettext
        except FileNotFoundError:
            _ = lambda s: s  # fallback — просто возвращает строку без перевода
...
class Localization(SetupLanguage):
    ...
    def init_window(self):
        ...
        for idx, (text, command) in enumerate(buttons):
            ...
            bg_color = {"Russian/Русский": "#4CAF50", "English/Английский": "#2196f3"}.get(text, "#f44336")

            tk.Button(frame, text=text, command=command, bg=bg_color, **style).grid(**grid)
