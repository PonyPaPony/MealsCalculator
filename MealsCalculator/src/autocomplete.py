...
class Autocomplete:
    ...
    def setup_autocomplete(self, entry_widget: tk.Entry, suggestions, listbox_widget: tk.Listbox):
        ...
        if any(not isinstance(x, tk.Widget) for x in [entry_widget, listbox_widget]):
            raise ValueError("Ошибка: entry_widget и listbox_widget должны быть классом tkinter")
