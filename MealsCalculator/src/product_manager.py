...
class ProductManagerGUI:
    def __init__(self, context: ProductContext, products, info_message=None, error_message=None):
    ...
    def open_add_products_window(self, root: tk.Tk | tk.Toplevel = None):
        ...
        win = self.factory.create_widgets(cls=tk.Toplevel, title=_("Добавить продукт"), size='300x150')
        win.protocol("WM_DELETE_WINDOW", lambda: self.factory.on_close(root=root, window=win))
        frame = self.builder.create_frame(window=win, grid={**self.settings.frame_grid})

        self.builder.create_label(frame, text=_("Название продукта:"), style={**self.settings.font_10},
                                  grid={**self.settings.label_grid})
        self.builder.create_label(frame, text=_("Калорийность:"), style={**self.settings.font_10},
                                  grid={**self.settings.label_grid, 'row': 1})

        entry_name = self.builder.create_entry(frame, grid={**self.settings.entry_grid})
        entry_kcal = self.builder.create_entry(frame, grid={**self.settings.entry_grid, 'row': 1})
        ...
        def submit():
            ...
            if name in self.products:
                ...
                return
            if not name:
                self.error_message(_("Ошибка",), _("Поле продукта не может быть пустым."))
                return
            ...
            if is_valid:
                ...
                self.info_message(_("Успех"), _("Добавлен {n} с калорийностью {kcal}.").format(n=name, kcal=kcal_value))
            else:
                self.error_message(_("Внимание"), _("Неверный формат калорийности."))
                return

        self.builder.create_button(frame, text=_("Сохранить"), command=submit,
                                   style={**self.settings.font_12, **self.settings.green},
                                   grid={**self.settings.button_grid, 'row': 2})
        self.builder.create_button(frame, text=_("Назад"),
    ...
    def open_del_products_window(self, root: tk.Tk | tk.Toplevel = None):
        self.factory.window_status(root, 'hide')
        win = self.factory.create_widgets(cls=tk.Toplevel, title=_("Удалить продукт"), size='400x350')
        win.protocol("WM_DELETE_WINDOW", lambda: self.factory.on_close(root=root, window=win))
        frame = self.builder.create_frame(window=win, grid={**self.settings.frame_grid})

        self.builder.create_label(frame, text=_("Продукт для удаления:"), style={**self.settings.font_10},
                                  grid={**self.settings.label_grid})
        entry = self.builder.create_entry(frame, grid={**self.settings.entry_grid})

        listbox = self.builder.create_listbox(frame, style={**self.settings.listbox_style},
        ...
        def delete():
            ...
            if not name:
                ...
                self.info_message(_("Успех"), _("Продукт {name} удалён.").format(name=name))
            else:
                self.error_message(_("Ошибка"), _("Продукт {name} не найден.").format(name=name))

        btn_frame = self.builder.create_frame(win, grid={**self.settings.frame_grid, 'row': 1, 'sticky': 'ew'})
        self.builder.create_button(btn_frame, text=_("Удалить"), command=delete,
                                   style={**self.settings.font_12, **self.settings.red},
                                   grid={**self.settings.listbox_button})
        self.builder.create_button(btn_frame, text=_("Назад"),
                                   command=lambda: self.factory.restore_root_window(root=root, window=win),
    ...
    def open_change_products_window(self, root: tk.Tk | tk.Toplevel = None):
        self.factory.window_status(root, 'hide')
        win = self.factory.create_widgets(cls=tk.Toplevel, title=_("Изменить калорийность"), size='400x350')
        win.protocol("WM_DELETE_WINDOW", lambda: self.factory.on_close(root=root, window=win))
        frame = self.builder.create_frame(window=win, grid={**self.settings.frame_grid})

        self.builder.create_label(frame, text=_("Продукт:"), style={**self.settings.font_10},
                                  grid={**self.settings.label_grid})
        self.builder.create_label(frame, text=_("Новая калорийность:"), style={**self.settings.font_10},
                                  grid={**self.settings.label_grid, 'row': 1})

        entry_product = self.builder.create_entry(frame, grid={**self.settings.entry_grid})
        entry_kcal = self.builder.create_entry(frame, grid={**self.settings.entry_grid, 'row': 1})
        ...
        def update():
            ...
            if not name:
                ...
                self.error_message(_("Ошибка"), _("Продукт {n} не найден.").format(n=name))
            kcal = entry_kcal.get()
            if not kcal:
            ...
            if is_valid:
                self.update_product_data(name, kcal_value)
                self.info_message(_("Успех"), _("Калорийность {n} обновлена.").format(n=name))

        btn_frame = self.builder.create_frame(win, grid={**self.settings.frame_grid, 'row': 3, 'sticky': 'ew'})
        self.builder.create_button(btn_frame, text=_("Изменить"), command=update,
                                   style={**self.settings.font_12, **self.settings.green},
...
class ProductManager(ProductManagerGUI):
    ...
    def validate_product_input(self, name: str, kcal: str) -> tuple[bool, Optional[float]]:
        """
        ...
        try:
            ...
            self.error_message(_("Ошибка"), _("Калорийность {kcal} не является числом.").format(kcal=kcal))
            return False, None
...
class ProductCalculator(ProductManagerGUI):
    ...
    def calculate_total(self, data):
        ...
        for product_var, weight_var in data:
            ...
            if name not in self.products:
                raise ValueError(_("Продукт '{name}' не найден в базе.").format(name=name))
            try:
    ...
    def create_input_product_row(self, data, frame: tk.Frame):
        ...
        box = self.builder.create_combobox(frame, textvariable=product_var, values=list(self.products.keys()),
                                     style={**self.settings.combo_style},
                                     grid={**self.settings.combo_grid, "row": row_index})

        entry = self.builder.create_entry(frame, textvariable=weight_var, style={**self.settings.font_10},
        ...
        btn = self.builder.create_button(frame, text=_("Удалить"), command=remove_row,
